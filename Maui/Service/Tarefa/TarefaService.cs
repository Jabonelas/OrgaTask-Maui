...
namespace Maui.Service
{
    internal class TarefaService : ITarefaService
    {
        ...
        public async Task<(bool Sucesso, string ErrorMessagem)> CadastrarTarefaAsync(TarefaDTO _dadosTarefa)
        {
            try
            {
        ...
        public async Task<(bool Sucesso, string ErrorMessagem)> AlterarTarefaAsync(TarefaDTO _dadosTarefa)
        {
            try
            {
                UserToken dadosToken = await PegarDadosToken();
        ...
        public async Task<(bool Sucesso, string ErrorMessagem, TarefaDTO TarefaAlterarDTO)> BuscarTarefaAsync(int _id)
        {
            try
            {
                UserToken dadosToken = await PegarDadosToken();
                ...
                using (var request = new HttpRequestMessage(HttpMethod.Get, endpoint))
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        var tarefa = JsonConvert.DeserializeObject<TarefaDTO>(responseContent);
